...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int write = XSpi_Transfer(&SpiInstance, writeVal, NULL, 2);
	if(write != 0){
		xil_printf("Error");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	write = XSpi_Transfer(&SpiInstance, send, NULL, nbytes+1);
	if(write != 0){
			xil_printf("Error");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE val[2];
	val[0]=reg;
	int read = XSpi_Transfer(&SpiInstance, val, val, 2);
	if(read != 0){
		xil_printf("Error");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	recvbuf[0]=reg;
//	BYTE rg[1]={reg};
//	XSpi_Transfer(&SpiInstance, rg, NULL, 1);
	read = XSpi_Transfer(&SpiInstance, recvbuf, recvbuf, nbytes+1);
	if(read != 0){
				xil_printf("Error");
			}
	for(int n = 1; n < nbytes+1; n++){
		data[n-1]=recvbuf[n];
//		read = XSpi_Transfer(&SpiInstance, &reg, data + 8*n, 1);
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
}
